<script lang="ts">
    ...
    // Keep the canvas pin layer's console mode in sync. Invisible pins are
    // painted only while the console is open (they stay hit-testable either
    // way), so this must re-run whenever the console toggles or the layer is
    // rebuilt.
    $effect(() => {
        void layerRevision;
        pinLayer?.setConsoleOpen(isConsoleOpen);
    });
    ...
    function initializeMap(config: MapConfig) {
        ...
        //
        // 1. Y axis goes DOWN (not up) matching image pixel coordinates and the
    ...
    function finishDrawing() {
        ...
        if (!mapConfig) {
            log.error(
                "Cannot finish drawing: Map config not found.",
                undefined,
                "MapView",
            );
            return;
