<script lang="ts">
    ...
    const ZOOM_MIN = 0.1;
    const ZOOM_MAX = 4;
    /** Screen-px margin left around the content by "zoom to fit". */
    ...
    /** Frame the given nodes' bounding box, never zooming past 1:1. */
    function fitTo(nodes: RectLike[], rect: DOMRect): void {
    ...
    function resetZoom(): void {
        zoomAboutCenter(1 / viewport.zoom);
    }

    function zoomToFit(): void {
        fitTo(canvas.nodes, containerRect());
    }

    /** Write through the store and refresh the toolbar's undo/redo flags.
     *  Failures are logged and rolled back inside canvasStore. */
    function applyUpdate(fn: (d: CanvasData) => CanvasData) {
    ...
    function undo() {
        ...
        applyUpdate((d) => history.undo(d) ?? d);
    }
    function redo() {
        if (!history.canRedo()) return;
        applyUpdate((d) => history.redo(d) ?? d);
    }
    ...
    function endGesture() {
        ...
        history.push(before);
        syncHistory();
        // Persist whatever the queue's read step finds, not the snapshot taken
        // above: a mutation already queued behind a slow write commits after
        // that snapshot, and writing `current` would silently revert it. The
        // preview mutations have already put this gesture's result in the
        // cache, so the identity updater writes exactly that, plus anything
        // that landed in between.
        updateCanvas(path, (d) => d).catch(() => {
    ...
    /**
     * Runs the active creation tool at `at`, reverting to the select tool —
     * creation tools are one-shot. Returns false if no card was placed
     * because the active tool doesn't create anything.
     */
    function placeWithActiveTool(at: Point): boolean {
        switch (tool) {
            case "text":
                createTextCard(at);
                break;
    ...
    function createTextCard(at: Point) {
        ...
        const node: CanvasNodeData = {
            id,
            type: "text",
            x: Math.round(at.x - TEXT_CARD_WIDTH / 2),
            y: Math.round(at.y - TEXT_CARD_HEIGHT / 2),
        ...
        openModal({
            ...
            props: {
                ...
                onSelect: async (filename: string) => {
                    ...
                    if (!abs || !root) return;
                    // Size the card to the image's natural aspect ratio. If
                    // the dimensions can't be read they stay at zero, and
                    // defaultImageCardSize supplies its generic fallback.
                    let dims = { width: 0, height: 0 };
                    try {
                        const src = await getImageSource(abs);
                        dims = await detectImageDimensions(src);
                    } catch (e) {
                        log.warn(
                    ...
                    const size = defaultImageCardSize(dims.width, dims.height);
                    createFileCard(
                        at,
    ...
    /** Shift-click toggles membership; a plain click on an unselected node
    ...
    >
        ...
        <div class="zoom-control">
            <button
                ...
                onclick={() => zoomAboutCenter(1 / 1.2)}>−</button
            >
            <button
                class="pct"
            ...
            <button
                ...
                onclick={() => zoomAboutCenter(1.2)}>+</button
            >
            <button
