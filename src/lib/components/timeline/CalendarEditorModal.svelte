            ...
            {#if def.leapRules.length > 0}
                <div class="leap-grid">
                    ...
                    {#each def.leapRules as rule, i (i)}
                        <Select
                            ariaLabel={$t("timeline.leapMonthLabel")}
                            options={def.months.map(
                                (m: { name: string }, mi: number) => ({
                                    value: String(mi),
                                    label: m.name,
                                }),
                            )}
                            value={String(rule.monthIndex)}
                            onSelect={(v: string) =>
                                (rule.monthIndex = Number(v))}
                        />
                        <input
                            type="number"
                            min="1"
            ...
            {#if def.eras.length > 0}
                <div class="era-grid">
                    ...
                    {#each def.eras as era, i (i)}
                        ...
                        />
                        <Select
                            ariaLabel={$t("timeline.eraDirectionLabel")}
                            options={[
                                {
                                    value: "1",
                                    label: $t("timeline.eraCountsUp"),
                                },
                                {
                                    value: "-1",
                                    label: $t("timeline.eraCountsDown"),
                                },
                            ]}
                            value={String(era.direction)}
                            onSelect={(v: string) =>
                                (era.direction = Number(v) as 1 | -1)}
                        />
                        <Button
                            variant="ghost"
                            class="row-remove"
...
<style>
    ...
    /* The shared trigger is sized for full-width forms; these grid rows are
       compact, so pin it to the inputs beside it. */
    .editor :global(.select-trigger) {
        padding: 0.35rem 0.55rem;
        border-radius: 4px;
        background: var(--color-background-primary);
        font-size: 0.9rem;
