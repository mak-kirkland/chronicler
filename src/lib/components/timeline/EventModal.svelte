<script lang="ts">
    ...
    // Seeded once, at mount: the modal is remounted per open, and re-deriving
    // the draft would discard whatever the user has typed.
...
>
    <form onsubmit={submit} class="form">
        ...
        <!-- Not a wrapping <label>: SelectTrigger is a button, and a label
             wrapping it would open the dropdown on every label click. -->
        <div class="field">
            <span class="field-label">{$t("timeline.laneLabel")}</span>
            <Select
                options={timeline.lanes.map(
                    (lane: { id: string; name: string }) => ({
                        value: lane.id,
                        label: lane.name || $t("timeline.unnamedLane"),
                    }),
                )}
                bind:value={draft.laneId}
                ariaLabel={$t("timeline.laneLabel")}
            />
        </div>
        <fieldset>
...
<style>
    ...
    /* Matches the <label> stacks around it. */
    .field {
        display: flex;
        flex-direction: column;
        gap: 0.25rem;
    ...
    /* The shared trigger is sized for full-width forms; these rows are
       compact, so pin it to the inputs beside it. */
