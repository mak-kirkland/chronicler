...
<Modal title={$t("timeline.newTitle")} {onClose}>
    <form onsubmit={handleSubmit} class="form">
        ...
        <div class="form-group">
            <!-- SelectTrigger takes no id, so a `for` here would do nothing
                 but let a label click open the dropdown. -->
            <!-- svelte-ignore a11y_label_has_associated_control -->
            <label>{$t("timeline.calendarLabel")}</label>
            <Select
                options={choices.map((c: { id: string; name: string }) => ({
                    value: c.id,
                    label: c.name,
                }))}
