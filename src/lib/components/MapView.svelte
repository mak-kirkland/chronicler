<script lang="ts">
    ...
    let drawMode = $state<"polygon" | "circle" | null>(null);
        ...
        try {
            ...
            if (!map) {
                ...
                map.on("contextmenu", (e: L.LeafletMouseEvent) => {
                    ...
                    const shapesAtCursor = getShapesAt(mapX, mapY);
                ...
                map.on("mousemove", (e: L.LeafletMouseEvent) => {
                    ...
                    const shapes = getShapesAt(mapX, mapY);
                ...
                map.on("click", (e: L.LeafletMouseEvent) => {
                    if (!isDrawing) {
                        ...
                        const shapes = getShapesAt(mapX, mapY).filter(
    ...
    // --- Geometry Helpers for Overlaps ---

    function getShapesAt(mapX: number, mapY: number): MapRegion[] {
        if (!mapConfig || !mapConfig.shapes) return [];

        return mapConfig.shapes.filter((shape) => {
            if (shape.type === "circle") {
                const dx = mapX - shape.x;
                const dy = mapY - shape.y;
    ...
    function startDrawing(mode: "polygon" | "circle") {
        isDrawing = true;
        drawMode = mode;
        tempPoints = [];
