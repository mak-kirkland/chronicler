<script lang="ts">
    ...
    onMount(() => {
        // The snippets watcher normally keeps this list live, but it is allowed
        // to fail to start (live reload is non-critical). Re-scanning on open
        // means the list is still correct when it did.
    ...
    <div class="snippets-body">
        ...
        {:else}
            <div class="snippet-list">
                {#each $snippets as snippet (snippet.filename)}
                    <ToggleSwitch
                        id={`snippet-toggle-${snippet.filename}`}
