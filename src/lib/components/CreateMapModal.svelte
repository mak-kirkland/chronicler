<script lang="ts">
    ...
    // Default dimensions, will be updated when image is selected
    let mapWidth = $state(2048);
    let mapHeight = $state(2048);
    ...
    $effect(() => {
        ...
        if (fullPath) {
            ...
            img.onerror = (err) => {
                console.warn(
                    `[CreateMapModal] Failed to load image for dimension detection:`,
                    err,
                );
            };
    ...
    async function handleSubmit(event?: Event) {
        ...
        if (!name || !selectedImage || !$vaultPath) {
            console.warn("CreateMapModal: Missing fields", {
                name,
                selectedImage,
                vaultPath: $vaultPath,
            });
...
<Modal title="Create New Map" {onClose}>
    <form onsubmit={handleSubmit} class="form">
        ...
        <div class="form-group">
            ...
            />
            <p class="help-text">Select an image.</p>
            {#if selectedImage}
                <p class="help-text dimension-text">
                    Detected Dimensions: {mapWidth} x {mapHeight} px
                </p>
            {/if}
        </div>
        ...
        <div class="modal-actions">
            ...
            <Button
                type="submit"
                disabled={!name || !selectedImage || isCreating}
            >
