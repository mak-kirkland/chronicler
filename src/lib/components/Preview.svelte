    ...
    function interactiveContent(node: HTMLElement, htmlContent: string) {
        ...
        function initialize(content: string) {
            ...
            tables.forEach((table) => {
                ...
                const hasMergedCells = table.querySelector(
                    "[colspan], [rowspan]",
                );

                // Tabulator does not support merged cells
                // Skip it and leave it as a plain HTML table.
                if (hasMergedCells) {
                    return;
                }
...
<style>
    ...
    /* Header section */
    .main-content :global(.tabulator .tabulator-header .tabulator-col) {
        background-color: var(--color-background-tertiary);
        color: var(--color-text-secondary);
    }

    /* Header hover */
    .main-content :global(.tabulator .tabulator-header .tabulator-col:hover) {
    ...
    /*
     * This provides the base styling for rows, targeting the 'odd' rows
     * to use the primary background color.
     */
    .main-content :global(.tabulator .tabulator-row.tabulator-row-odd) {
        background-color: var(--color-background-primary);
        color: var(--color-text-primary);
    }

    /*
     * This targets the 'even' rows to use the secondary background color,
     * creating the alternating (striped) effect.
     */
    .main-content :global(.tabulator .tabulator-row.tabulator-row-even) {
        background-color: var(--color-background-secondary);
        color: var(--color-text-primary);
    }
