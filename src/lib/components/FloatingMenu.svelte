<script lang="ts">
    ...
    function updatePosition() {
        ...
        // using window dimensions. A more robust solution might use `tick`
        // to check menuEl dimensions first.
        ...
        if (menuEl) {
            ...
            if (top + menuRect.height > innerHeight) {
                ...
                // For now, just shifting up slightly or clamping
    ...
    function handleGlobalEvents() {
        ...
        // If resizing or scrolling, we generally want to close
        // OR recalculate. Most of your original code closed it.
        // We will stick to closing for safety, or recalculate if it's just a scroll.
        // The original components varied: SmartInput/Autocomplete closed on scroll.
        onClose();
    }
...
{#if isOpen}
    <div
        ...
        style="position: fixed; {calculatedStyle} z-index: 1000; {style}"
    >
        {@render children()}
    </div>
{/if}
