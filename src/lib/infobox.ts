...
export function applyInfoboxStateToContent(
    ...
    if (state.layoutRules.length > 0) {
        ...
        // but inner arrays (keys: [A, B]) to be Flow style.
        const layoutNode = doc.createNode(layoutData);
        if (isSeq(layoutNode)) {
            layoutNode.items.forEach((item) => {
                if (isMap(item)) {
                    // Find 'keys' property and set flow=true
                    const keysNode = item.get("keys", true) as Node; // 'true' gets the node wrapper
                    if (keysNode && isSeq(keysNode)) {
                        keysNode.flow = true;
                    }
                }
            });
    ...
    // A. Update or Add fields from Editor
    ...
    // B. Detect Deletions (Non-Destructive)
    ...
    if (isMap(doc.contents)) {
        ...
        for (const key of existingKeys) {
            ...
            // If it's in the editor, we already updated it.
