...
export type CanvasNodeData = CanvasTextNode | CanvasFileNode | CanvasGroupNode;
...
/**
 * The active toolbar tool. Not part of the file format — it lives here so the
 * view, the toolbar and each node agree on one union.
 */
export type CanvasTool = "select" | "text" | "image" | "embed" | "connect";
...
export function genNodeId(): string {
    // uuid() (not crypto.randomUUID directly): the latter is undefined on
    // older WKWebView where the Tauri scheme is not a secure context.
    return uuid();
}
