...
describe("createTranslator: fallback chain", () => {
    ...
    // A blank value must behave like a missing one, or the per-key fallback
    // silently renders an empty label — the one thing it exists to prevent.
    it("treats an empty string as missing and falls back to English", () => {
        const blank = createTranslator(
            { en, pl: { ...pl, "settings.title": "" } },
            "pl",
        );
        expect(blank("settings.title")).toBe("Settings");
    });

    it("treats an empty plural form as missing and falls back to English", () => {
        const blank = createTranslator(
            {
                en,
                pl: { ...pl, "importer.imported": { one: "", other: "" } },
            },
            "pl",
        );
        expect(blank("importer.imported", { count: 1 })).toBe(
            "1 file imported",
        );
    });
});
