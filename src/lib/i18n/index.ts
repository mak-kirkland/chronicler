...
/**
 * The `{name}` placeholder grammar, shared with the locale validation tests
 * so they check the syntax the runtime actually substitutes.
 */
export const PLACEHOLDER_RE = /\{(\w+)\}/g;

/** The placeholder names a template expects, in source order. */
export function placeholderNames(template: string): string[] {
    return [...template.matchAll(PLACEHOLDER_RE)].map(([, name]) => name);
}
...
function pickForm(
    ...
    // An empty string is treated as absent throughout: a translator who
    // blanks a value gets the English text, never an empty label.
    ...
    const category =
        typeof count === "number"
            ...
            : "other";
    return forms[category] || forms.other || undefined;
}
...
/**
 * Every bundled locale, keyed by lowercased code. Exported so the locale
 * validation tests check the same catalog the runtime serves, rather than
 * re-deriving it from the filenames and drifting.
 */
export const dictionaries: Record<string, LocaleDict> = {};
...
// The translator for the active locale, kept current by the subscription
// below. `t` only rebuilds this when a component happens to be subscribed,
// so holding our own reference keeps `translate()` cheap during startup and
// in the vault-selector/error paths, where nothing is mounted yet.
let currentTranslator: TranslateFn = createTranslator(dictionaries, "en");
...
export function translate(key: string, params?: TranslationParams): string {
    return currentTranslator(key, params);
}
...
locale.subscribe((code) => {
    currentTranslator = createTranslator(dictionaries, code);
