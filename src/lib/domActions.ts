...
/**
 * A Svelte action that finds all spans with class 'math' (generated by pulldown-cmark)
 * and renders the LaTeX content using KaTeX.
 * @param node The container element to scan for math spans.
 * @param data The rendered page data (as a dependency to trigger updates).
 */
export function renderMath(node: HTMLElement, data: any) {
    const process = () => {
        if (!data) return;

        // pulldown-cmark renders math as <span class="math math-inline">...</span>
        // or <span class="math math-display">...</span>
        const mathElements = node.querySelectorAll(".math");
        mathElements.forEach((el) => {
            if (el.getAttribute("data-processed")) return;

            const tex = el.textContent || "";
            // 'math-display' is for $$...$$, 'math-inline' is for $...$
            ...
            try {
                katex.render(tex, el as HTMLElement, {
                    ...
                    throwOnError: false,
                    trust: true, // Allow styling commands
                });
    ...
    return {
        update(newData: any) {
            data = newData;
            // Wait briefly for Svelte to inject the new HTML string from {@html}
            // into the DOM before trying to find the new .math elements.
