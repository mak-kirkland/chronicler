...
import type { CanvasEdgeSide } from "./canvasModels";
...
export function marqueeHitTest(rect: WorldRect, nodes: RectLike[]): string[] {
...
const SIDE_DIR: Record<CanvasEdgeSide, Point> = {
    top: { x: 0, y: -1 },
    bottom: { x: 0, y: 1 },
    left: { x: -1, y: 0 },
    right: { x: 1, y: 0 },
...
function sideAnchor(n: RectLike, side: CanvasEdgeSide): Point {
    switch (side) {
        case "top":
...
function autoSides(
    a: RectLike,
    b: RectLike,
): { from: CanvasEdgeSide; to: CanvasEdgeSide } {
