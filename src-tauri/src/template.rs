...
//! which are stored in a special `.chronicler/templates` directory within the
...
pub fn get_all(app_handle: &AppHandle) -> Result<Vec<PageHeader>> {
...
#[instrument]
pub fn get_content(path: &Path) -> Result<String> {
...
pub fn save_content(app_handle: &AppHandle, name: &str, content: &str) -> Result<PathBuf> {
    let templates_dir = get_templates_dir(app_handle)?;
