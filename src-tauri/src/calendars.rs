...
use crate::config::VAULT_CONFIG_DIR_NAME;
use crate::error::{ChroniclerError, Result};
use crate::utils::{ensure_dir, list_files_with_extension};
use std::fs;
use std::path::{Path, PathBuf};

/// The vault-relative directory calendars live in.
pub fn calendars_dir(vault_root: &Path) -> PathBuf {
    vault_root.join(VAULT_CONFIG_DIR_NAME).join("calendars")
}

fn ensure_calendars_dir(vault_root: &Path) -> Result<PathBuf> {
    Ok(ensure_dir(calendars_dir(vault_root))?)
}
...
pub fn list_calendar_jsons(vault_root: &Path) -> Result<Vec<String>> {
    Ok(
        list_files_with_extension(&calendars_dir(vault_root), "json")?
            .into_iter()
