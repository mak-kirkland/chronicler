...
use crate::{
    ...
    renderer::Renderer,
    snippets::{start_snippet_watcher, SnippetWatcher},
    utils::{
...
impl World {
    ...
    fn initialize(&self, root_path: &Path, app_handle: AppHandle) -> Result<()> {
        ...
        let new_snippet_watcher = start_snippet_watcher(root_path, app_handle.clone());
        ...
        // The lock scope is kept as short as possible.
        {
            ...
            *self.snippet_watcher.lock() = new_snippet_watcher;
    ...
    ///
    /// The frontend awaits this before mounting a layer so a pre-cached
    /// pyramid skips the full-resolution image fallback entirely.
    pub fn lookup_layer_tile_info(
        ...
        Ok(image_path.and_then(|p| crate::tiler::lookup_tile_info(&root, &p)))
    }

    /// Generates (or returns cached) tile pyramid data for a map layer image.
    ///
    /// Delegates to `tiler::generate_tiles_async` which runs the
    /// CPU-bound work on a blocking thread so progress events can be
